...
FILE* Storage::getHandle(long index) {
...
void Storage::closeActiveHandle(long index) {
...
void Storage::loadCache() {
    ...
    for (long index = 0; index < dataIndex; index++) {
        ...
        memset(keyValue, 0, size * KEY_VALUE_SIZE); // unwritten tail (level not fully grown yet) reads back as empty slots
...
void Storage::insert(long dataIndex, map<prf_type, prf_type> ciphers, bool setupMode, bool firstInsert) {
    ...
    if (setupMode) {
        ...
        if (!firstInsert) {
            ...
            memset(wholeFile, 0, KEY_VALUE_SIZE * maxSize); // unwritten tail (level not fully grown yet) reads back as empty slots
