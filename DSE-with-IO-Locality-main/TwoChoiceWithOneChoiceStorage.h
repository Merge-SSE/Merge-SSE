...
private:
    // One handle per level, opened lazily on first access and kept open until the
    // object is destroyed (see Storage.h for why: closing/reopening a handle on
    // every level switch turns each query into dozens of fopen/fclose calls).
    vector<FILE*> handles;
