...
static void resetRootDirectory(const string& path) {
#ifdef _WIN32
    system(("rmdir /s /q \"" + path + "\" >nul 2>nul").c_str());
    system(("mkdir \"" + path + "\"").c_str());
#else
    system(("rm " + path + " -rf").c_str());
    system(("mkdir " + path).c_str());
#endif
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 4) {
        if (argc == 2 && string(argv[1]) == "test") {
            resetRootDirectory(Utilities::rootAddress);
            ...
            cout << "Usage: dse SCHEME_NAME HARDWARE CACHE_SIZE [N_UPDATES] [CONFIG_FILE] or dse test" << endl << endl;
            ...
            cout << "N_UPDATES (optional, default 100): \t" << "number of update queries to run and average" << endl << endl;
            cout << "CONFIG_FILE (optional, default config.txt): \t" << "path to the config file (see generate_config.py)" << endl << endl;
        }
        return 0;
    }
    schemeName = argv[1];
    ...
    int N_updates = argc > 4 ? atoi(argv[4]) : 100;

    string resultFilename = "result_" + sanitizeForFilename(schemeName) + "_" + hardware
            + "_cache" + to_string(cacheSize) + "_u" + to_string(N_updates) + ".txt";
    ...
    bool setup = true;
    string filename = argc > 5 ? string(argv[5]) : "config.txt";
    Utilities::readConfigFile(argc, argv, filename, testCases);
    std::vector<pair<string, int> > tests;
    std::vector<int> ops; //0=ins 1=del
    resetRootDirectory(Utilities::rootAddress);
#ifdef _WIN32
    _setmaxstdio(8192); // raise the CRT's open-FILE* cap to its Windows-enforced maximum (default is 512)
#else
    ...
    scheme->endSetup();
    double setupTime = Utilities::stopTimer(600);
    cout << "End of setting up initial key-value pairs" << endl;
    cout << "Total Setup Time (microseconds):" << setupTime << endl;
    resultFile << "setup," << setupTime << endl;
    ...
    for (uint j = 0; j < testCases[0].Qs.size(); j++) {
        ...
        for (int z = 0; z < 1; z++) {
            ...
            resultFile << testCases[0].Qs[j] << "," << scheme->totalSearchTime << endl;
            assert(testCases[0].Qs[j] - testCases[0].delNumber[j] == res.size());
            cout << "-------------------------------------------------" << endl;
        }
    }

    if (schemeName != "L-SDd[NlogN]" && schemeName != "L-SDd[3N]" && schemeName != "L-SDd[6N]" && cacheSize == 0 && N_updates > 0) {
        double totalUpdateTimeSum = 0;
        for (int u = 0; u < N_updates; u++) {
            string updateKeyword = "Test-KW-" + to_string(u);
            Utilities::startTimer(500);
            scheme->update(OP::INS, updateKeyword, 100000 + u, false);
            time = Utilities::stopTimer(500);
            totalUpdateTimeSum += scheme->totalUpdateTime;
            resultFile << "update," << scheme->totalUpdateTime << endl;
            auto res = scheme->search(updateKeyword);
            assert(res.size() == 1);
        }
        cout << "Average Update Computation Time over " << N_updates << " updates (microseconds):" << (totalUpdateTimeSum / N_updates) << endl;
        resultFile << "update_avg," << (totalUpdateTimeSum / N_updates) << endl;
    }
    resultFile.close();
    return 0;
}
