...
#ifdef _WIN32
std::string Utilities::rootAddress = "dse_data/";
#else
...
#ifdef _WIN32
// No portable Windows equivalent of these Linux device/kernel cache-drop commands
// (hdparm/nvme-cli/procfs are all Linux-only). Left as no-ops rather than failing:
// system("") runs no command and reports success, whereas the original commands
// would spawn cmd.exe once per query/update and fail with "cannot find the path
// specified" every time. This matches S1C/D1C, which also don't drop the OS page
// cache between accesses, so this keeps the comparison methodology consistent.
std::string Utilities::HDD_DROP_CACHE_COMMAND = "";
std::string Utilities::SSD_DROP_CACHE_COMMAND = "";
std::string Utilities::KERNEL_DROP_CACHE_COMMAND = "";
#else
...
void Utilities::fseek64(FILE* f, long long offset, int origin) {
#ifdef _WIN32
    _fseeki64(f, offset, origin);
#else
    fseeko(f, (off_t) offset, origin);
#endif
}

long long Utilities::ftell64(FILE* f) {
#ifdef _WIN32
    return _ftelli64(f);
#else
...
// Creates `filename` (or, if forceNew, truncates it to empty) without writing any data.
// Replaces shelling out to `dd`/`/dev/zero` (not available on Windows, where it silently
// no-ops, leaving the file missing) and, per explicit direction, does NOT zero-fill the
// file up front to its eventual full size: callers that need a level's full capacity to
// read back as zero for never-written slots rely on the OS's normal sparse-hole behavior
// (seeking past EOF and writing implicitly zero-fills the gap on both Windows and Linux),
// so the file only grows as data is actually written to it.
void Utilities::ensureFileExists(std::string filename, bool forceNew) {
    if (forceNew) {
        FILE* trunc = fopen(filename.c_str(), "wb");
        if (!trunc) {
            cerr << "Error creating file " << filename << ": " << strerror(errno) << endl;
            return;
        }
        fclose(trunc);
        return;
    ...
    FILE* created = fopen(filename.c_str(), "wb");
    if (!created) {
        cerr << "Error creating file " << filename << ": " << strerror(errno) << endl;
        return;
    }
