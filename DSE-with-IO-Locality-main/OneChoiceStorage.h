...
private:
    // One handle per level, opened lazily on first access and kept open until
    // closeActiveHandle(index) is called explicitly (by closeHandle/rename/resetup,
    // when that level's file is about to be rewritten/renamed/deleted) or the object
    // is destroyed. The amortized scheme touches O(log N) levels per query, so
    // closing and reopening a handle on every level switch (as a single shared
    // handle would require) turns each query into dozens of fopen/fclose calls.
    vector<FILE*> handles;
    FILE* getHandle(long index);
    void closeActiveHandle(long index);
