...
def plot_throughput(D1C_throughputs):
    xs = [ii for ii in range(len(D1C_throughputs))]
    xlabels = []
    ys = []
    for page_size in sorted(D1C_throughputs.keys()):
        N_items = (page_size -32)//16
        xlabels += [str(page_size) + f'({N_items})']

        ys += [D1C_throughputs[page_size]]
...
plot_throughput(D1C_throughputs)
