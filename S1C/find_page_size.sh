...
#   ./S1C.exe -f <input> --fp -p <page_size> [-d <delta>]
#
...
for page_size in $page_sizes; do
    log_file="logs/S1C_optp_${page_size}.log"
    ...
    echo "Running S1C --fp: page_size=$page_size ${delta:+delta=$delta} ..." >&2
    start_time=$(date +%s)

    set +e
    ./S1C.exe "${args[@]}" > "$log_file" 2>&1
    exit_code=$?
    set -e
...
echo ""
echo "Results written to ../benchmarks/S1C-opt-p/ -- plot with ../benchmark_plots/benchmark_page_size_S1C.py"
