...
void CuckooHasing::Build_hash_table_enc_and_fill(std::unordered_map<size_t, byte_t*> raw_table, unsigned int block_len,
    ...
    if (verbose_setup)
        std::cout << "Build_hash_table_enc_and_fill: block_len=" << block_len << " hash_table_size=" << hash_table_size
                   << " raw_table.size()=" << raw_table.size() << std::endl;

    double load_factor = (double) raw_table.size() / (2.0 * hash_table_size);
    if (load_factor >= 1.0) {
        std::cerr << "Build_hash_table_enc_and_fill: FATAL - " << raw_table.size() << " items cannot fit in a cuckoo hash table"
        ...
        std::cerr << "Build_hash_table_enc_and_fill: WARNING - load factor " << load_factor * 100 << "% for " << raw_table.size()
    ...
    while (succeed == false) {
        ...
        if (verbose_setup && (attempt % 1000 == 0 || attempt <= 5))
            std::cout << "Build_hash_table_enc_and_fill: attempt " << attempt << " (hash_table_size=" << hash_table_size
        ...
        if (loop_ctr < LOOP_MAX) {
            ...
            std::cerr << "Build_hash_table_enc_and_fill: FATAL - load factor " << load_factor * 100 << "% for " << raw_table.size()
            ...
            std::cout << "Build_hash_table_enc_and_fill: attempt " << attempt << " failed (hit LOOP_MAX=" << LOOP_MAX
                       << ", hash_table_size=" << hash_table_size << ", n_items=" << raw_table.size() << "), retrying" << std::endl;
        }
    }

    if (verbose_setup)
        std::cout << "Build_hash_table_enc_and_fill: succeeded after " << attempt << " attempt(s) (hash_table_size=" << hash_table_size
                   << ", n_items=" << raw_table.size() << ")" << std::endl;
